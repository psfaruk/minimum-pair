...
async def run(engine: str, candles: list[dict], learn: bool) -> dict:
    ...
    tally = {"confirmed": [0, 0], "fallback": [0, 0]}  # tier -> [wins, losses]
    draws = skipped = 0
    ...
    total_w = tally["confirmed"][0] + tally["fallback"][0]
    total_l = tally["confirmed"][1] + tally["fallback"][1]
    return {
        "engine": engine,
        "bars": len(clean),
        ...
        "win_rate_fallback": rate("fallback"),
        "n_fallback": tally["fallback"][0] + tally["fallback"][1],
...
async def main() -> None:
    ...
    print(f"  win_rate_fallback  = {result['win_rate_fallback']} (n={result['n_fallback']})")
