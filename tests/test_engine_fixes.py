...
Covers:
  ...
  5. decision noise path — the ensemble's own net direction is used as
     the filler direction instead of being thrown away.
  6. decision lone-vote rule — a single unmeasured vote fires noise,
...
def test_noise_uses_ensemble_direction():
    """A below-quality-floor vote pool must still steer the noise signal
    by its own net direction, not by last-candle colour."""
    ...
    # fired noise signal must carry the ensemble's direction (CALL, the
    # reversion read), NOT the colour-follow PUT.
    assert dec.tier == "noise", f"lone unmeasured vote must be noise, got {dec.tier}"
    assert dec.direction == "CALL", f"noise should carry the ensemble's direction, got {dec.direction}"
    assert dec.sources == ["rsi_oversold"]
    print("  decision.noise-uses-ensemble OK")
...
def test_lone_vote_cannot_confirm():
    ...
    # Below LONE_VOTE_MIN_WEIGHT -> noise; at/above -> confirmed happens
    # in evaluate(); here we assert the threshold logic directly.
...
if __name__ == "__main__":
    ...
    test_noise_uses_ensemble_direction()
